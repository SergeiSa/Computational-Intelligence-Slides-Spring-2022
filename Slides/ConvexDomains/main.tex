...
This time, it is the fact that one of the \emph{boundaries} of the domain was not included. This makes problem 6 to have no solution, while problem 7 has a solution. For problem 6 we can pick an arbitrary value close to $x = 2$, approaching it from the left, but for any such value, there always will be other values of the decision variable closer to $x = 2$ and hence producing larger values of $f$.
